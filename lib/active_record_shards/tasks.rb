...
namespace(:db) do
  ...
  task(abort_if_pending_migrations: :environment) do
    if defined?(ActiveRecord)
      pending_migrations = if ActiveRecord::VERSION::MAJOR >= 6
    ...
    class << self
      ...
      def spec_for(conf)
        ...
        when "7.0"
          ActiveRecord::Base
            .connection_handler
            .send(:resolve_pool_config, conf, ActiveRecord::Base, ActiveRecord::Base.current_role, ActiveRecord::Base.current_shard)
        when "6.1"
